...
- **Título do trabalho**: _Carrier Birds Simulator_
Será implementado um simulador no qual aves transportam objetos de um ponto inicial até um destino definido, desviando de obstáculos, como árvores, paredes e estruturas sólidas, e atravessando ambientes com superfícies flexiveis como folhagem, rios, lago, nuvens. O usuário poderá controlar o voo da ave e o momento de entrega do objeto. Serão implementadas colisões entre o jogador e outras aves, bem como em relação ao ambiente.
...
Elementos de obstáculos, arvores, nuvens, prédios simples baseados no castelo visto no vídeo, serão instanciados múltiplas vezes no cenário.
...
Alguns movimentos da ave pode ser automatizado para a aplicação das curvas. No entanto, se um dos objetos for leve suficiente como carta, deve haver um deslizamento suave. Outras aves podem ter sua rota de movimento implementada através de curvas de Bézier.
...
- **Física de voo** não iremos implementar toda a aerodinâmica de voar, mas quanto mais perto melhor. (_implementação parcial_)
- **Animação das asas** o ideal seria simular a ação do vento nas penas e ajustes naturais enquanto a ave voa, mas isso exigiria maior complexidade. (_implementação parcial_)
- **Aplicação de dano** quando objeto cair de uma altura maior do que o esperado será aplicado uma deformação simbolica. (_implementação parcial_)
- **Cenário total** não será implementado o cenário inteiro visto nos vídeos/imagens, mas tanto quanto for possível. (_implementação parcial_)
