...
{
    generate(); // chama buildMesh, computeNormals, setupBuffers
}

void HouseDrawable::setPosition(const glm::vec3& position)
{
...
void HouseDrawable::buildMesh()
{
    ...
    // Corpo da casa (coordenadas locais, centrado na origem)
...
void HouseDrawable::draw(const DrawContext& ctx)
{
...
{
    // Como a geometria é simples e as normais não são críticas,
    // podemos definir todas apontando para cima (ou calcular corretamente)
