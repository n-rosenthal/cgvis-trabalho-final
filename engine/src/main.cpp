...
#include "UIRenderer.hpp"       //  UI, painel debug etc
#include "ResourceManager.hpp"  //  gerenciamento de recursos (shaders, texturas, modelos)
...
// outras informações do programa. Definidas após main(). ---> delegado à UIRenderer.cpp
...
{
    auto& rm = ResourceManager::get();
    auto& virtualModels = rm.getVirtualModels();   // mapa de VirtualModel
    ...
    for (size_t shape = 0; shape < model->shapes.size(); ++shape)
    {
        ...
        VirtualModel vm;
        vm.name           = model->shapes[shape].name;
        vm.first_index    = first_index;
        vm.num_indices    = last_index - first_index + 1;
        vm.rendering_mode = GL_TRIANGLES;
        vm.vertex_array_object_id = vertex_array_object_id;
        vm.bbox_min = bbox_min;
        vm.bbox_max = bbox_max;

        virtualModels[vm.name] = vm;
    }

    // -------------------------------------------------------------------------
    // Upload dos dados para a GPU (VBOs)
    // -------------------------------------------------------------------------
